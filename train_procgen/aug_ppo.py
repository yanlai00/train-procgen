"""
Combine all previous tricks and achieve sample efficiency 
General idea is to use a fewer number of parallel environment (to compare with benchmark runs),
but apply different augmenting methods and stack up each input batch
and the network sees num_env * N_augs observations and output more actions than needed,
so we randomly segment one out of the N_augs action sets and choose that as next actions
"""
...
N_augs = 3 ## how many aug. tricks to use
N_Envs = 16 
...
class AugCnnPolicy(CnnPolicy): ## Not considering color_transform!
...
def crosscut(obs, shape=(32, 32)): ##obs.shape==num_evns,64,64,3
    """
...
def jitter(obs):
    ...
    #     c = im.copy()
    #     c[:,:,0] += jitter
    #     plt.imsave("jitter{}.jpg".format(i), c)
    # cpy[:,:,:,0] += jitter
    return obs
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, nsteps, gamma, lam):
        ...
        self.lam = lam
        self.gamma = gamma
        #logger.info("before cut has shape", self.obs.shape)
        self.obs =augment(self.obs)
        self.dones = np.concatenate([self.dones for _ in range(N_augs+1)], axis=0)
        logger.info("Runner init has obs shape: ", self.obs.shape)
        for i in range(2):
           plt.imsave("aug1_{}.jpg".format(str(i)), self.obs[i]) #Sanity check!
           plt.imsave("aug2_{}.jpg".format(str(i+16)), self.obs[i+16]) #Sanity check!
           plt.imsave("aug3_{}.jpg".format(str(i+32)), self.obs[i+32])
           plt.imsave("aug4_{}.jpg".format(str(i+48)), self.obs[i+48])
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            #logger.info("step with upsized input has action shape: ", actions.shape)
            
            # values = values[N_Envs*idx: N_Envs*(idx+1)]
            ...
            idx = np.random.randint(N_augs+1) ## NOTE: randomly choose action
            act = actions[N_Envs*idx: N_Envs*(idx+1)]
            obs, rewards, dones, infos = self.env.step(act)
            #print(dones.shape)
            self.obs[:] = augment(obs)
            cum_rewards = np.concatenate([rewards for _ in range(N_augs+1)], axis=0)
            self.dones[:] = np.concatenate([dones for _ in range(N_augs+1)], axis=0)
            
            #logger.info("in one run step obs shape: ", self.obs[:].shape)
            
            for info in infos:
                maybeepinfo = info.get('episode')
                if maybeepinfo: epinfos.append(maybeepinfo)
            mb_rewards.append(cum_rewards)
        ...
        for t in reversed(range(self.nsteps)):
            if t == self.nsteps - 1:
                ...
                nextvalues = last_values#[N_Envs*idx: N_Envs*(idx+1)]
            else:
                nextnonterminal = 1.0 - mb_dones[t+1]
                nextvalues = mb_values[t+1]#[N_Envs*idx: N_Envs*(idx+1)]
            # print(mb_rewards[t].shape)
            # print(mb_values[t].shape)
...
def learn(*, network, sess, env, nsteps, total_timesteps, ent_coef, lr,
    ...
    nbatch = nenvs * nsteps * (N_augs+1) ## NOTE: upsize!!
    
    nbatch_train = nbatch // nminibatches
    policy = AugCnnPolicy
    logger.info("in learn: env.num_envs", env.num_envs, "\n")
    logger.info("in learn: nbatch: ", nbatch, "\n")
    logger.info("in learn: nbatch_train: ", nbatch_train, "\n")
    nbatch_act = nenvs*(N_augs+1)  ## NOTE: upsize!!
    model = Model(policy=policy, sess=sess, ob_space=ob_space, ac_space=ac_space, 
        nbatch_act=nbatch_act, nbatch_train=nbatch_train,
